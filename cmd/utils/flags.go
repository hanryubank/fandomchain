...
var (
	...
	FandomTestnetFlag = cli.BoolFlag{
		Name:  "fandom-testnet",
		Usage: "Fandom test network",
	}
	...
	TargetGasLimitFlag = cli.Uint64Flag{
		...
		Value: params.FandomGenesisGasLimit,
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(FandomTestnetFlag.Name):
		urls = params.TestnetBootnodes
	}
...
// for fandom and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `fandom account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(StoreRewardFlag.Name) {
		common.StoreRewardFolder = filepath.Join(stack.DataDir(), "fandom", "rewards")
...
// new format.
//
// e.g. fandom account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// fandom --keystore /tmp/mykeystore --lightkdf account new
//
