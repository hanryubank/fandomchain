...
func airDropTokenToAccountNoFandom() {
...
func testTransferTRC21TokenWithAccountNoFandom() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have fandom
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoFandom()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoFandom()
		fmt.Println("Finish transfer trc21 token with a account no fandom")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no fandom")
	}
