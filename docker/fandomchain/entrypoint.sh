...
params=""
accountsCount=$(
  fandom account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    1222 )
      ...
      params="$params --fandom-testnet --gcmode archive --rpcapi db,eth,net,web3,debug"
      ;;
    1223 )
...
# data dir
if [[ ! -d $DATA_DIR/fandom ]]; then
  echo "No blockchain data, creating genesis block."
  fandom init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    fandom  account import ./private_key \
      --datadir $DATA_DIR \
...
]' > $DATA_DIR/fandom/static-nodes.json
